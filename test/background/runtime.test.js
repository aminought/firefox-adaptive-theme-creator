...
describe("Runtime", () => {
  describe("makeColors", () => {
    ...
    const tests = [
      makeParams(
        "should return theme color when global disabled and part disabled",
        [makePartParams("frame", THEME_COLOR)]
      ),
      makeParams(
        "should return theme color when global disabled and part enabled",
        [makePartParams("frame", THEME_COLOR, { enabled: true })]
      ),
      makeParams(
        "should return theme color when global enabled and part disabled",
        [makePartParams("frame", THEME_COLOR)],
        { globalEnabled: true }
      ),
      makeParams(
        "should return global color when enabled and inheritance is global",
        [
      ...
      makeParams(
        "should return one color for all inherited parts",
        [
          makePartParams("frame", "rgb(0 0 0)", {
            enabled: true,
            color: "rgb(0 0 0)",
      ...
      makeParams(
        "should return one color for all inherited parts when order is reverted",
        [
          makePartParams("tab_selected", "rgb(0 0 0)", {
            enabled: true,
            inheritance: "sidebar",
      ...
      makeParams(
        "should return two colors for two groups of inherited parts",
        [
          makePartParams("frame", "rgb(0 0 0)", {
            enabled: true,
            color: "rgb(0 0 0)",
      ...
      makeParams(
        ...
        { globalEnabled: true, faviconMostPopularColor: null }
      ),
    ];
    tests.forEach((test) => {
      it(test.id, () => {
        ...
        const actualColors = runtime.makeColors(
          parts,
          test.faviconMostPopularColor
            ? new Color(test.faviconMostPopularColor)
            : null,
          test.pageMostPopularColor
            ? new Color(test.pageMostPopularColor)
            : null
        );

        expect(Object.keys(actualColors).sort()).to.deep.equal(
          Object.keys(parts).sort()
        );
        for (const partParams of test.parts) {
          expect(actualColors[partParams.name].css()).to.equal(
            partParams.expectedColor
          );
        }
      });
    });
